...
def parse_fight_page(html: str, url: str) -> dict:
    ...
    for round_num, fighter_idx, stats in _iter_round_stats(totals_table, TOTALS_STATS):
        stats_by_round_fighter[(round_num, fighter_idx)] = stats
    for round_num, fighter_idx, stats in _iter_round_stats(sig_str_table, SIG_STR_STATS):
        ...
        round_stats.append(
            {
                ...
                "td_landed": td_landed,
                "td_att": td_att,
                "sub_att": _to_int(stats.get("sub_att")),
                "rev": _to_int(stats.get("rev")),
